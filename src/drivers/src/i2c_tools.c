...
static void i2c_bus_recover(
  ...
  // go back to alternate open-drain on sda and scl lines
...
{
  if (hi2c == &hi2c1) {
    i2c_bus_recover(hi2c, GPIOB, GPIO_PIN_6, GPIO_PIN_7, GPIO_AF4_I2C1);
    unlock_count_1++;
  } else if (hi2c == &hi2c2) {
    i2c_bus_recover(hi2c, GPIOB, GPIO_PIN_10, GPIO_PIN_11, GPIO_AF4_I2C2);
    unlock_count_2++;
  }
}
